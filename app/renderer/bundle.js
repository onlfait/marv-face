...
function getCamera(settings = {}) {
  settings = cloneDeep_1(defaultSettings, settings);
...
function cameraWatcher({ timeout = 2000, ...settings } = {}) {
  ...
  function waitForCamera() {
    ...
    getCamera(settings)
      .then(camera => {
        events$1.emit("camera", camera);
        camera.track.addEventListener("ended", () => {
          events$1.emit("ended", camera);
          waitForCamera();
        });
      })
      .catch(error => {
        events$1.emit("error", error);
  ...
  return {
    ...
    once: events$1.once.bind(events$1)
  };
}
